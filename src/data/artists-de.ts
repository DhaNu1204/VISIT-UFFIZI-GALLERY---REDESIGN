import type { Artist } from "./artists";
export type ArtistI18n = Omit<Artist, "slug">;
export const artistsDe: Record<string, ArtistI18n> = {
  "sandro-botticelli": {
    name: "Sandro Botticelli",
    title: "Sandro Botticelli in den Uffizien | Geburt der Venus",
    description:
      "Botticellis Geburt der Venus und Primavera in den Uffizien. Raumführer, Meisterwerke und Biografie des beliebtesten Malers von Florenz.",
    h1: "Sandro Botticelli in den Uffizien",
    born: "ca. 1445, Florenz",
    died: "17. Mai 1510, Florenz",
    movement: "Frührenaissance",
    ufficiRoom: "Räume 10-14 (Botticelli-Saal)",
    masterpieces: [
      "Geburt der Venus (ca. 1485)",
      "Primavera (ca. 1482)",
      "Anbetung der Könige (ca. 1475)",
      "Madonna des Magnificat (ca. 1481)",
      "Pallas und der Zentaur (ca. 1482)",
    ],
    intro:
      "Betreten Sie die Räume 10-14 der Uffizien und Sie werden sofort verstehen, warum man diesen Bereich den Botticelli-Saal nennt. Die Geburt der Venus befindet sich direkt vor Ihnen -- und ja, sie ist wirklich so schön in Wirklichkeit. Kein Foto, kein Druck, kein Bildschirm kann die leuchtende Qualität von Botticellis Farben oder die schiere Grösse dieses Gemäldes einfangen. Mit fast 3 Metern Breite dominiert Venus, die auf ihrer Muschel aus dem Meer aufsteigt, den Raum wie eine Vision.",
    atTheUffizi:
      "Der Botticelli-Saal ist das emotionale Herz der Uffizien. Die Galerie widmet diesem einen Künstler fünf miteinander verbundene Räume (10-14) -- eine Ehre, die dem wichtigsten Maler des Museums vorbehalten ist. Die Geburt der Venus und die Primavera stehen sich von gegenüberliegenden Wänden gegenüber und schaffen einen Dialog zwischen zwei der bekanntesten Gemälde der westlichen Kunst.\n\nDie Geburt der Venus (ca. 1485) zeigt die Göttin bei ihrer Ankunft an der Küste von Zypern, angeblasen von den Windgöttern Zephyr und Chloris. Die Figur der Venus ist einer antiken römischen Skulptur entlehnt -- der Venus Pudica (schamhafte Venus) -- aber Botticelli verwandelt sie in etwas Ätherisches. Ihr unmöglich langer Hals, das wallende goldene Haar und die schwerelos wirkende Pose schaffen eine Figur, die ausserhalb normaler Anatomie zu existieren scheint. Das ist beabsichtigt. Botticelli malte nicht die Realität -- er malte ein Ideal.\n\nDie Primavera (ca. 1482) ist das komplexere Werk. Über 500 Pflanzenarten wurden in dem Gemälde botanisch identifiziert, alle mit wissenschaftlicher Genauigkeit dargestellt. Die neun Figuren -- Merkur, die drei Grazien, Venus, Amor, Flora, Chloris und Zephyr -- bilden eine Allegorie des Frühlings, über die Gelehrte seit Jahrhunderten diskutieren. Achten Sie auf Amor direkt über Venus, mit verbundenen Augen, der seinen Pfeil auf die mittlere Grazie richtet.\n\nVerpassen Sie nicht die Anbetung der Könige -- Botticelli malte sich selbst in die Menge als jungen Mann in einem gelben Gewand, der Sie vom rechten Bildrand direkt anblickt. Mehrere Mitglieder der Familie Medici sind ebenfalls unter den Königen dargestellt.",
    biography:
      "Alessandro di Mariano di Vanni Filipepi -- der Welt bekannt als Sandro Botticelli -- wurde um 1445 in Florenz geboren. Sein Spitzname \"Botticelli\" (kleines Fass) stammte von seinem älteren Bruder. Er lernte bei Fra Filippo Lippi, einem der führenden Maler seiner Zeit, und entwickelte schnell einen unverwechselbaren Stil, der sich durch fliessende Linien, anmutige Figuren und leuchtende Farben auszeichnete.\n\nBotticellis Karriere wurde durch seine enge Beziehung zur Familie Medici geprägt. Lorenzo de' Medici (Lorenzo der Prächtige) war sein grösster Mäzen und gab Werke in Auftrag, die klassische Mythologie mit neuplatonischer Philosophie verbanden. Die Geburt der Venus und die Primavera entstanden beide aus diesem intellektuellen Kreis -- es sind nicht nur schöne Gemälde, sondern komplexe philosophische Aussagen über Liebe, Schönheit und das Wesen des Göttlichen.\n\nIn den 1490er Jahren geriet Botticelli unter den Einfluss des leidenschaftlichen Dominikanerpredigers Savonarola, der die Eitelkeit der weltlichen Kunst verurteilte. Laut Giorgio Vasari warf Botticelli 1497 einige seiner eigenen Gemälde auf Savonarolas \"Fegefeuer der Eitelkeiten\". Seine späteren Werke wurden zunehmend religiös und streng. Er starb 1510, weitgehend vergessen, und wurde in der Kirche Ognissanti begraben -- nur einen kurzen Spaziergang von den Uffizien entfernt.",
    legacy:
      "Botticelli war nach seinem Tod fast 300 Jahre lang vergessen. Es waren die Präraffaeliten im England des 19. Jahrhunderts, die sein Werk wiederentdeckten, fasziniert von den gleichen fliessenden Linien und der ätherischen Schönheit, die Besucher auch heute noch begeistern. Walter Paters berühmter Essay über Botticelli von 1870 trug dazu bei, eine Wiederentdeckung auszulösen, die nie aufgehört hat. Heute ist die Geburt der Venus eines der am häufigsten reproduzierten Bilder der Welt -- und das Original im Botticelli-Saal zu sehen, bleibt eines der eindrucksvollsten Erlebnisse in Florenz.",
    faq: [
      {
        q: "Wo befinden sich Botticellis Gemälde in den Uffizien?",
        a: "Botticellis Hauptwerke befinden sich in den Räumen 10-14, dem sogenannten Botticelli-Saal. Dies ist der beliebteste Bereich der Uffizien und kann zwischen 10 und 14 Uhr sehr voll werden.",
      },
      {
        q: "Wie viele Botticelli-Gemälde gibt es in den Uffizien?",
        a: "Die Uffizien besitzen etwa 20 Werke von Botticelli und damit die weltweit grösste Sammlung. Die berühmtesten sind die Geburt der Venus, Primavera, Anbetung der Könige und Madonna des Magnificat.",
      },
      {
        q: "Worum geht es in der Geburt der Venus?",
        a: "Das Gemälde zeigt die Göttin Venus bei ihrer Ankunft an der Küste von Zypern, nachdem sie aus dem Meeresschaum geboren wurde. Sie wird von den Windgöttern Zephyr und Chloris ans Ufer geblasen, während die Hore des Frühlings wartet, um sie zu bekleiden. Es ist eine Allegorie der göttlichen Schönheit, die in die sterbliche Welt eintritt.",
      },
    ],
  },
  "michelangelo": {
    name: "Michelangelo",
    title: "Michelangelo in den Uffizien | Doni Tondo - Raum 35",
    description:
      "Michelangelos Doni Tondo in den Uffizien, Raum 35. Sein einziges Tafelbild -- leuchtende Farben, die die Sixtinische Kapelle vorwegnehmen.",
    h1: "Michelangelo in den Uffizien",
    born: "6. März 1475, Caprese, Toskana",
    died: "18. Februar 1564, Rom",
    movement: "Hochrenaissance",
    ufficiRoom: "Raum 35",
    masterpieces: ["Doni Tondo (Heilige Familie) (ca. 1507)"],
    intro:
      "Michelangelo betrachtete sich in erster Linie als Bildhauer, erst in zweiter als Maler. Dass von seiner Hand nur ein einziges Tafelbild existiert, macht den Doni Tondo in Raum 35 der Uffizien zu etwas Aussergewöhnlichem -- einem seltenen Fenster in den Geist des Künstlers, der später die Decke der Sixtinischen Kapelle malen sollte. Stehen Sie davor, und die Farben treffen Sie zuerst: elektrische Rostöne, brillante Blaus, saure Grüntöne. Dies sind nicht die gedämpften Töne typischer Renaissance-Malerei. Dies sind die Farben einer Revolution.",
    atTheUffizi:
      "Der Doni Tondo (ca. 1507) hängt in Raum 35 neben den Werken Leonardo da Vincis -- ein Raum, der zwei Titanen der Renaissance in direkten Vergleich bringt. Das runde Format (Tondo) war in Florenz für häusliche Andachtsbilder beliebt, aber Michelangelo verwandelte es in etwas, das niemand zuvor gesehen hatte.\n\nDie Heilige Familie -- Maria, Josef und das Jesuskind -- dreht sich in einer komplexen Spiralkomposition. Maria greift hinter sich, um das Christuskind von Josef zu empfangen, und erzeugt so eine dynamische, spiralförmige Energie, die durch das gesamte Gemälde pulsiert. Die muskulösen Körper, die skulpturale Modellierung, die kühne Verkürzung -- alles hier nimmt die Decke der Sixtinischen Kapelle vorweg, mit der Michelangelo nur ein Jahr später beginnen sollte.\n\nSchauen Sie sich die Farben genau an. Sie waren nach einer Restaurierung im Jahr 1985 noch überraschender, als die ursprüngliche Brillanz unter jahrhundertealtem Firnis und Schmutz zum Vorschein kam. Die gleiche Kontroverse brach aus, als die Decke der Sixtinischen Kapelle in den 1990er Jahren gereinigt wurde -- es stellte sich heraus, dass Michelangelo immer ein Kolorist war, nicht der gedämpfte Maler, den man vermutet hatte.\n\nDer Originalrahmen ist ebenso bemerkenswert wie das Gemälde. Geschnitzt mit Groteskenköpfen, Pflanzenmotiven und fünf geschnitzten Köpfen (möglicherweise Propheten), wurde er vermutlich von Michelangelo selbst entworfen -- einer der schönsten Renaissance-Rahmen überhaupt.",
    biography:
      "Michelangelo di Lodovico Buonarroti Simoni wurde 1475 in Caprese geboren, einem kleinen toskanischen Ort. Seine Familie zog nach Florenz, als er noch jung war, und die Stadt prägte ihn zutiefst. Er ging kurzzeitig beim Maler Domenico Ghirlandaio in die Lehre, aber es war Lorenzo de' Medici, der sein Talent erkannte und den jugendlichen Michelangelo in den Medici-Haushalt einlud, wo er klassische Skulptur im Medici-Garten studierte.\n\nFlorenz gab der Welt Michelangelo, aber seine grössten Werke entstanden in Rom: die Pieta (1499), die Decke der Sixtinischen Kapelle (1508-1512), das Jüngste Gericht (1536-1541) und der Entwurf des Petersdoms. Dennoch betrachtete er sich stets als Florentiner. Der David (1504), aus einem Marmorblock gemeisselt, den zwei andere Bildhauer aufgegeben hatten, steht in der Galleria dell'Accademia in Florenz -- und der Doni Tondo in den Uffizien bleibt sein persönlichstes Gemälde.",
    legacy:
      "Michelangelo wurde 88 Jahre alt -- ein aussergewöhnliches Alter für das 16. Jahrhundert -- und arbeitete bis zu seinen letzten Lebenstagen. Er war der erste Künstler, dessen Biografie noch zu seinen Lebzeiten veröffentlicht wurde (von Giorgio Vasari im Jahr 1550), und er wurde bereits von seinen Zeitgenossen \"Il Divino\" (Der Göttliche) genannt. Sein Einfluss auf die westliche Kunst ist unermesslich -- vom Barock über Rodin bis zur modernen Skulptur bleibt seine Vision des menschlichen Körpers als Ausdruck spiritueller Kraft der Massstab.",
    faq: [
      {
        q: "Welches Gemälde von Michelangelo ist in den Uffizien?",
        a: "Der Doni Tondo (Heilige Familie), ca. 1507, in Raum 35. Es ist Michelangelos einziges vollendetes Tafelbild -- er arbeitete hauptsächlich als Bildhauer und Freskenmaler.",
      },
      {
        q: "Warum ist der Doni Tondo so bedeutend?",
        a: "Es ist das einzige vollendete Tafelbild von Michelangelo und nimmt die Decke der Sixtinischen Kapelle mit seinen kühnen Farben, skulpturalen Figuren und komplexen Kompositionen vorweg. Der Originalrahmen wurde möglicherweise ebenfalls von Michelangelo entworfen.",
      },
      {
        q: "Ist Michelangelos David in den Uffizien?",
        a: "Nein. Der David befindet sich in der Galleria dell'Accademia, etwa 15 Gehminuten von den Uffizien entfernt. Sie können beide Museen mit einer Kombinationstour besuchen.",
      },
    ],
  },
  "leonardo-da-vinci": {
    name: "Leonardo da Vinci",
    title: "Leonardo da Vinci in den Uffizien | Verkündigung",
    description:
      "Leonardos Verkündigung und Anbetung der Könige in den Uffizien. Raum-35-Führer und frühe Florentiner Meisterwerke.",
    h1: "Leonardo da Vinci in den Uffizien",
    born: "15. April 1452, Vinci, Toskana",
    died: "2. Mai 1519, Amboise, Frankreich",
    movement: "Hochrenaissance",
    ufficiRoom: "Raum 35",
    masterpieces: [
      "Verkündigung (ca. 1472-1475)",
      "Anbetung der Könige (ca. 1481, unvollendet)",
      "Taufe Christi (ca. 1475, mit Verrocchio)",
    ],
    intro:
      "Leonardo war kaum 20 Jahre alt, als er die Verkündigung malte, die heute in Raum 35 der Uffizien hängt. Selbst in diesem Alter war sein Genie unverkennbar. Betrachten Sie die Flügel des Engels -- es sind nicht die stilisierten Fächer der mittelalterlichen Kunst, sondern realistische Vogelflügel, die Leonardo anhand tatsächlicher Exemplare studiert hatte, die er sezierte. Betrachten Sie die Landschaft hinter Maria -- diese dunstige, blaugrüne Ferne, die sich in Atmosphäre auflöst, ist Leonardos charakteristisches Sfumato, eine Technik, die er in den nächsten vier Jahrzehnten perfektionieren sollte.",
    atTheUffizi:
      "Raum 35 vereint Leonardo mit Michelangelo und schafft eine der aussergewöhnilichsten Konfrontationen der Kunstgeschichte. Leonardos Verkündigung dominiert die Wand mit ihrer ruhigen Präzision, während Michelangelos Doni Tondo vor muskulöser Energie pulsiert.\n\nDie Verkündigung (ca. 1472-1475) zeigt den Erzengel Gabriel bei seinem Besuch bei der Jungfrau Maria, um ihr zu verkünden, dass sie den Sohn Gottes gebären wird. Leonardo setzt die Szene in einen idealisierten Garten mit einem Marmor-Lesepult, das von einem Grabmal seines Lehrers Verrocchio kopiert wurde. Die Perspektive ist akribisch berechnet -- versuchen Sie, das Gemälde von der rechten Seite zu betrachten, und die Proportionen fügen sich perfekt zusammen. Leonardo hat es möglicherweise für die Betrachtung aus einem bestimmten Winkel in seiner ursprünglichen Kirche entworfen.\n\nDie Anbetung der Könige (ca. 1481) ist faszinierend unvollendet. Leonardo brach sie ab, als er 1482 Florenz in Richtung Mailand verliess, aber was erhalten ist, offenbart seinen revolutionären Arbeitsprozess. Die zugrunde liegende braune Tuschkomposition zeigt seine Besessenheit mit Bewegung, Emotion und der Geometrie von Gruppen. Pferde bäumen sich auf, Figuren gestikulieren, und die gesamte Szene spiralt um die zentrale Figur der Madonna. Das Gemälde ist im Wesentlichen eine Röntgenaufnahme von Leonardos arbeitendem Geist.\n\nAchten Sie auch auf die Taufe Christi, die in der Werkstatt von Andrea del Verrocchio entstanden ist. Leonardo malte den Engel ganz links und Teile der Landschaft. Laut Vasari war Verrocchio, als er Leonardos Engel sah, so beschämt von der Überlegenheit seines Schülers, dass er schwor, nie wieder zu malen.",
    biography:
      "Leonardo wurde 1452 in der kleinen toskanischen Stadt Vinci geboren. Als unehelicher Sohn eines Notars konnte er nicht in die Fussstapfen seines Vaters treten -- ein Schicksalsschlag, der ihn zur Kunst trieb. Er ging in Florenz bei Andrea del Verrocchio in die Lehre, wo er neben Botticelli, Perugino und Lorenzo di Credi ausgebildet wurde.\n\nFlorenz gab Leonardo seinen Start, aber den grössten Teil seiner Karriere verbrachte er anderswo -- in Mailand, Rom und schliesslich Frankreich, wo er 1519 als Gast von König Franz I. starb. Er hinterliess weniger als 20 vollendete Gemälde, aber seine Notizbücher -- über 7.000 Seiten mit Zeichnungen, wissenschaftlichen Beobachtungen und Erfindungen -- offenbaren einen Geist, der Kunst, Anatomie, Ingenieurwesen, Optik, Geologie, Botanik und Flug umfasste.",
    legacy:
      "Leonardos Gemälde in den Uffizien repräsentieren seine Florentiner Periode -- das Fundament, auf dem seine späteren Meisterwerke (die Mona Lisa, das Letzte Abendmahl, die Felsgrottenmadonna) aufgebaut wurden. Wenn Sie die Verkündigung persönlich sehen, können Sie einem 20-jährigen Genie dabei zusehen, wie er zum grössten Künstler aller Zeiten wird.",
    faq: [
      {
        q: "Welche Leonardo-Gemälde gibt es in den Uffizien?",
        a: "Die Uffizien besitzen drei Werke von oder zugeschrieben an Leonardo: die Verkündigung (ca. 1472-1475), die unvollendete Anbetung der Könige (ca. 1481) und seinen Beitrag zu Verrocchios Taufe Christi. Alle befinden sich in Raum 35.",
      },
      {
        q: "Ist die Mona Lisa in den Uffizien?",
        a: "Nein. Die Mona Lisa befindet sich im Louvre in Paris. Die Uffizien besitzen Leonardos frühere Werke aus seiner Florentiner Periode, darunter die Verkündigung und die unvollendete Anbetung der Könige.",
      },
      {
        q: "Wie alt war Leonardo, als er die Verkündigung malte?",
        a: "Leonardo war etwa 20-23 Jahre alt, als er die Verkündigung malte (ca. 1472-1475). Er arbeitete damals noch in Verrocchios Werkstatt in Florenz.",
      },
    ],
  },
  "giotto": {
    name: "Giotto di Bondone",
    title: "Giotto in den Uffizien | Ognissanti-Madonna - Raum 2",
    description:
      "Giottos revolutionäre Ognissanti-Madonna in den Uffizien, Raum 2. Wie ein Maler mit der mittelalterlichen Kunst brach und die westliche Malerei erfand.",
    h1: "Giotto in den Uffizien",
    born: "ca. 1267, Vespignano, Florenz",
    died: "8. Januar 1337, Florenz",
    movement: "Proto-Renaissance",
    ufficiRoom: "Raum 2",
    masterpieces: ["Ognissanti-Madonna (ca. 1310)"],
    intro:
      "In Raum 2 der Uffizien beginnt die westliche Malerei. Drei riesige Altarbilder füllen den Raum -- gewaltige Goldgrund-Darstellungen der Madonna mit Kind von Cimabü, Duccio und Giotto. Auf den ersten Blick sehen sie ähnlich aus. Doch stellen Sie sich vor Giottos Ognissanti-Madonna (ca. 1310) und wenden Sie sich dann Cimabüs Version zu, und Sie werden die Revolution sehen, die die Kunst für immer veränderte. Giottos Madonna hat Gewicht. Sie sitzt auf einem echten Thron, der echte Tiefe erzeugt. Ihr Gesicht zeigt echte menschliche Emotion. Nach Giotto konnte die Malerei nie wieder flach sein.",
    atTheUffizi:
      "Raum 2 ist bewusst so gestaltet, dass Sie die Geburt der Renaissance-Malerei mit einem einzigen Blick erleben können. Die drei grossen Mästa-Tafeln -- von Cimabü (ca. 1280), Duccio (ca. 1285) und Giotto (ca. 1310) -- stellen drei Stufen der Revolution von mittelalterlicher zu moderner Kunst dar.\n\nGiottos Ognissanti-Madonna bricht in mehreren entscheidenden Punkten mit der byzantinischen Konvention. Der Thron ist mit konsistenter Perspektive dargestellt und erzeugt ein überzeugendes Gefühl dreidimensionalen Raums. Der Körper der Madonna hat echtes Volumen unter ihren Gewändern -- man spürt ihr Gewicht auf dem Thron. Die Engel, die den Thron flankieren, überlappen sich natürlich, anstatt in einem symbolischen Muster gestapelt zu sein. Und die Gesichter -- Maria, das Christuskind, die Engel -- drücken echte menschliche Emotionen aus anstatt der stilisierten Masken der byzantinischen Kunst.\n\nDas mag subtil klingen, aber im Jahr 1310 war es revolutionär. Tausend Jahre lang war die westliche Kunst im Wesentlichen flach und symbolisch gewesen. Giotto erfand im Alleingang die Idee, dass Malerei die reale, physische Welt darstellen kann. Jeder Künstler, der folgte -- Masaccio, Botticelli, Leonardo, Michelangelo -- baute auf dem Fundament auf, das Giotto in diesem Raum gelegt hat.",
    biography:
      "Giotto di Bondone wurde um 1267 in der Nähe von Florenz geboren. Die berühmteste Geschichte über seine Jugend stammt von Vasari: Der Maler Cimabü soll den jungen Giotto entdeckt haben, als dieser ein Schaf auf einen Felsen so realistisch zeichnete, dass Cimabü ihn sofort als Lehrling aufnahm. Ob diese Legende wahr ist oder nicht -- Giotto lernte bei Cimabü und übertraf seinen Meister schnell.\n\nSein grösstes Werk ist der Freskenzyklus in der Scrovegni-Kapelle in Padua (ca. 1305), der das Leben Christi und der Jungfrau Maria mit beispielloser emotionaler Tiefe und räumlichem Realismus erzählt. In Florenz entwarf er den Glockenturm (Campanile) der Kathedrale, der noch heute seinen Namen trägt. Dante schrieb in der Göttlichen Komödie, dass Giottos Ruhm den von Cimabü übertroffen habe -- eine bemerkenswerte Hommage eines Florentiner Genies an ein anderes.",
    legacy:
      "Giotto wird oft der \"Vater der westlichen Malerei\" genannt. Wenn Sie in Raum 2 der Uffizien stehen, können Sie genau sehen, warum. Die Ognissanti-Madonna ist der Angelpunkt zwischen mittelalterlicher und moderner Kunst -- und alles andere in den Uffizien folgt daraus.",
    faq: [
      {
        q: "Wo befindet sich Giottos Gemälde in den Uffizien?",
        a: "Giottos Ognissanti-Madonna (ca. 1310) befindet sich in Raum 2, zusammen mit ähnlichen Werken von Cimabü und Duccio. Es ist der erste grosse Raum, den Sie in der Galerie betreten.",
      },
      {
        q: "Warum ist Giotto so bedeutend?",
        a: "Giotto wird der \"Vater der westlichen Malerei\" genannt, weil er mit dem flachen, symbolischen Stil der mittelalterlichen Kunst brach und Naturalismus einführte -- echten Raum, echtes Gewicht, echte Emotion. Jeder Renaissance-Künstler baute auf seinen Innovationen auf.",
      },
      {
        q: "Wie alt ist Giottos Gemälde in den Uffizien?",
        a: "Die Ognissanti-Madonna stammt aus etwa 1310 -- über 700 Jahre alt. Es ist eines der ältesten und historisch bedeutsamsten Gemälde in den Uffizien.",
      },
    ],
  },
  "giorgio-vasari": {
    name: "Giorgio Vasari",
    title: "Giorgio Vasari | Architekt der Uffizien",
    description:
      "Giorgio Vasari entwarf die Uffizien und den berühmten Vasari-Korridor. Architekt, Maler und Autor der Künstlerbiografien.",
    h1: "Giorgio Vasari -- Architekt der Uffizien",
    born: "30. Juli 1511, Arezzo",
    died: "27. Juni 1574, Florenz",
    movement: "Manierismus",
    ufficiRoom: "Im gesamten Gebäude",
    masterpieces: [
      "Das Uffizien-Gebäude (1560-1580)",
      "Vasari-Korridor (1565)",
      "Die Leben der hervorragendsten Maler, Bildhauer und Architekten (1550/1568)",
    ],
    intro:
      "Jeder Besucher der Uffizien läuft durch Giorgio Vasaris grösstes Kunstwerk, ohne es zu bemerken -- das Gebäude selbst. Vasari entwarf die Uffizien 1560 als Verwaltungsbüros für Cosimo I. de' Medici und schuf eine elegante U-förmige Struktur, die eine der schönsten Piazzas von Florenz einrahmt. Fünf Jahre später baute er den aussergewöhnlichen erhöhten Korridor, der seinen Namen trägt und die Uffizien über den Ponte Vecchio mit dem Palazzo Pitti verbindet.",
    atTheUffizi:
      "Vasaris Präsenz in den Uffizien ist eher architektonischer als malerischer Natur. Das Design des Gebäudes -- mit seinen harmonischen Säulen, offenen Loggien und klassischen Proportionen -- gilt als eines der schönsten Beispiele bürgerlicher Renaissance-Architektur. Der lange Korridor im Obergeschoss, ursprünglich zur Ausstellung der wachsenden Kunstsammlung der Medici entworfen, wurde zur Vorlage für Museumsgalerien in ganz Europa.\n\nDer Vasari-Korridor, in erstaunlichen fünf Monaten im Jahr 1565 erbaut, bleibt eines der aussergewöhnlichsten Bauwerke von Florenz. Er führt von den Uffizien über die Oberseite des Ponte Vecchio zum Palazzo Pitti und ermöglichte es den Medici, sich zwischen ihren Residenzen zu bewegen, ohne einen Fuss auf eine öffentliche Strasse zu setzen. Heute ist er nach umfangreicher Restaurierung für Besucher geöffnet.\n\nVasari malte auch äusserst produktiv -- seine Werke hängen in verschiedenen Räumen der Uffizien, obwohl seine Gemälde oft von den Meistern überschattet werden, über die er schrieb. Sein Selbstporträt ist Teil der berühmten Sammlung von Künstler-Selbstporträts der Galerie.",
    biography:
      "Vasari wurde 1511 in Arezzo geboren und lernte in Florenz bei Andrea del Sarto und (kurzzeitig) Michelangelo. Er wurde ein vielseitiger und produktiver Künstler -- Maler, Architekt und Schriftsteller. Als Hofkünstler und Architekt der Medici beaufsichtigte er bedeutende Projekte, darunter die Renovierung des Inneren des Palazzo Vecchio, den Entwurf der Uffizien und den Bau des Vasari-Korridors.\n\nDoch Vasaris beständigstes Vermächtnis ist weder ein Gebäude noch ein Gemälde -- es ist ein Buch. Seine \"Leben der hervorragendsten Maler, Bildhauer und Architekten\" (erstmals 1550 veröffentlicht, 1568 erweitert) sind das Fundament der westlichen Kunstgeschichte. Ohne Vasaris Biografien von Giotto, Leonardo, Michelangelo, Raffael und Dutzenden anderer wäre vieles, was wir über die Renaissance-Kunst wissen, verloren.",
    legacy:
      "Vasari erfand die Kunstgeschichte als Disziplin. Sein Konzept, dass die Kunst vom \"Wiedergeburt\" (rinascita) klassischer Werte bis zur Vollkommenheit Michelangelos voranschritt, gab uns das Wort \"Renaissance\" selbst. Jedes seitdem geschriebene Kunstgeschichtslehrbuch steht in Vasaris Schuld.",
    faq: [
      {
        q: "Hat Vasari die Uffizien entworfen?",
        a: "Ja. Giorgio Vasari entwarf das Uffizien-Gebäude 1560 für Cosimo I. de' Medici. Ursprünglich als Verwaltungsbüros gedacht, wurde es später zur Galerie, die die Kunstsammlung der Medici beherbergt.",
      },
      {
        q: "Was ist der Vasari-Korridor?",
        a: "Ein erhöhter Gang, den Vasari 1565 erbaute und der die Uffizien über den Ponte Vecchio mit dem Palazzo Pitti verbindet. Er ermöglichte es den Medici, sich zwischen den Gebäuden zu bewegen, ohne öffentliche Strassen zu benutzen.",
      },
      {
        q: "Was hat Vasari ausser Architektur noch gemacht?",
        a: "Vasari schrieb die \"Leben der Künstler\" -- den grundlegenden Text der westlichen Kunstgeschichte. Er war auch ein produktiver Maler und renovierte das Innere des Palazzo Vecchio.",
      },
    ],
  },
  "raphäl": {
    name: "Raffael",
    title: "Raffael in den Uffizien | Madonna mit dem Stieglitz",
    description:
      "Raffaels Madonna mit dem Stieglitz in den Uffizien, Raum 66. Das Gemälde, das die Zerstörung überlebte -- Restaurierung und Florenz-Führer.",
    h1: "Raffael in den Uffizien",
    born: "6. April 1483, Urbino",
    died: "6. April 1520, Rom",
    movement: "Hochrenaissance",
    ufficiRoom: "Raum 66",
    masterpieces: [
      "Madonna mit dem Stieglitz (1506)",
      "Selbstporträt (ca. 1506)",
      "Papst Leo X. mit Kardinälen (1518-1519)",
    ],
    intro:
      "Raffael starb an seinem 37. Geburtstag. In diesen wenigen Jahren schuf er Werke von so vollkommener Harmonie und Anmut, dass er schon zu Lebzeiten \"der göttliche Maler\" genannt wurde. Die Uffizien besitzen mehrere bedeutende Raffaels, darunter die Madonna mit dem Stieglitz -- ein Gemälde mit einer der dramatischsten Überlebensgeschichten der Kunstgeschichte.",
    atTheUffizi:
      "Die Madonna mit dem Stieglitz (1506) in Raum 66 wurde als Hochzeitsgeschenk für Raffaels Freund Lorenzo Nasi gemalt. Sie zeigt die Jungfrau Maria, die in einer Landschaft sitzt, mit dem jungen Johannes dem Täufer, der dem Christuskind einen Stieglitz überreicht -- ein Symbol für Christi künftige Passion.\n\nIm Jahr 1547 zerstörte ein verheerender Erdrutsch Lorenzo Nasis Haus auf dem Hügel von San Giorgio. Das Gemälde wurde in 17 Fragmente zerschmettert. Nasis Sohn beauftragte sofort einen Restaurator, die Stücke zusammenzusetzen, aber der Schaden war schwer. Jahrhundertelang waren die Risse unter Schichten von Übermalungen sichtbar.\n\nIm Jahr 2008 wurde eine grosse Restaurierung unter Einsatz moderner Techniken wie Infrarot- und Röntgenanalyse abgeschlossen. Die Restauratoren entfernten jahrhundertealten Firnis und Übermalungen und legten Raffaels Originalfarben frei -- leuchtende Blaus, warme Rots und sanfte Grüntöne, die 400 Jahre lang verborgen gewesen waren. Wenn Sie sehr genau hinschaün, können Sie noch schwache Linien erkennen, wo die Fragmente zusammengefügt wurden.\n\nEbenfalls in den Uffizien: Raffaels jugendliches Selbstporträt und das imposante Papst Leo X. mit Kardinälen -- ein politisches Porträt des Medici-Papstes, das Raffaels psychologische Tiefe offenbart. Leo X. (Giovanni de' Medici) war der Sohn Lorenzo des Prächtigen, was dieses Gemälde zu einer direkten Verbindung zwischen der Medici-Sammlung und der Familie Medici macht.",
    biography:
      "Raffael wurde 1483 in Urbino geboren und studierte bei Perugino, bevor er 1504 nach Florenz zog, wo er die Neürungen von Leonardo und Michelangelo in sich aufnahm. Seine Florentiner Periode brachte die Madonnenbilder hervor, die seinen Ruf begründeten -- heitere, perfekt ausgewogene Kompositionen, die mühelos anmutig wirkten.\n\nIm Jahr 1508 berief Papst Julius II. Raffael nach Rom, um die Stanzen (Räume) des Vatikans auszumalen. Das Ergebnis -- insbesondere die Schule von Athen -- ist eine der höchsten Errungenschaften der westlichen Kunst. Raffael wurde zum begehrtesten Maler Europas und leitete eine grosse Werkstatt, die Aufträge für Fresken, Porträts, Altarbilder und architektonische Entwürfe übernahm. Er starb plötzlich 1520 in Rom im Alter von nur 37 Jahren.",
    legacy:
      "Raffael repräsentiert das Ideal der Hochrenaissance -- Schönheit, Harmonie und Anmut zur Vollkommenheit gebracht. Sein Einfluss auf die akademische Malerei hielt über Jahrhunderte an. Wenn Sie die Madonna mit dem Stieglitz in den Uffizien betrachten, erleben Sie sowohl sein Genie als auch die Zerbrechlichkeit künstlerischen Überlebens.",
    faq: [
      {
        q: "Welche Raffael-Gemälde gibt es in den Uffizien?",
        a: "Die Uffizien besitzen die Madonna mit dem Stieglitz (1506), Raffaels Selbstporträt, Papst Leo X. mit Kardinälen und mehrere weitere Werke. Sie befinden sich hauptsächlich in Raum 66.",
      },
      {
        q: "Was geschah mit der Madonna mit dem Stieglitz?",
        a: "Im Jahr 1547 zerstörte ein Erdrutsch das Haus, in dem sie hing, und zerschmetterte sie in 17 Stücke. Sie wurde zusammengesetzt, zeigte aber jahrhundertelang Schäden. Eine Restaurierung im Jahr 2008 legte Raffaels Originalfarben frei.",
      },
      {
        q: "Wie alt war Raffael, als er starb?",
        a: "Raffael starb an seinem 37. Geburtstag (6. April 1520) in Rom. Trotz seines kurzen Lebens schuf er einige der einflussreichsten Gemälde der westlichen Kunst.",
      },
    ],
  },
  "titian": {
    name: "Tizian",
    title: "Tizian in den Uffizien | Venus von Urbino - Raum 83",
    description:
      "Tizians Venus von Urbino in den Uffizien, Raum 83. Der berühmteste liegende Akt der Kunstgeschichte -- Analyse und Besucherführer.",
    h1: "Tizian in den Uffizien",
    born: "ca. 1488, Pieve di Cadore, Venetien",
    died: "27. August 1576, Venedig",
    movement: "Venezianische Renaissance",
    ufficiRoom: "Raum 83",
    masterpieces: [
      "Venus von Urbino (1538)",
      "Flora (ca. 1515)",
      "Porträt der Eleonora Gonzaga della Rovere (1536-1537)",
    ],
    intro:
      "Tizian war der Meister der Farbe. Während die Florentiner Maler ihre Kunst auf Zeichnung und Linie aufbauten, gründeten Tizian und die venezianische Schule die ihre auf den sinnlichen Reichtum der Ölfarbe. Nirgends wird dies deutlicher als in der Venus von Urbino (1538) in Raum 83 -- einem Gemälde, das so provokativ war, dass Mark Twain es als \"das unanständigste, gemeinste, obszönstes Bild, das die Welt besitzt\" bezeichnete. Er lag falsch. Es ist eines der grössten.",
    atTheUffizi:
      "Die Venus von Urbino dominiert Raum 83. Eine nackte Frau liegt auf einem Bett und blickt den Betrachter mit ruhiger Selbstsicherheit direkt an. Ein kleiner Hund schläft zu ihren Füssen (Symbol der Treü), Dienerinnen durchsuchen eine Hochzeitstruhe im Hintergrund, und eine Myrte steht auf der Fensterbank (mit Venus und daürhafter Liebe assoziiert).\n\nWas dieses Gemälde aussergewöhnlich macht, ist nicht die Nacktheit -- es ist die vollständige Beherrschung der Ölfarbe. Tizian baut die Figur mit Schichten durchscheinender Farbe auf und erzeugt Haut, die von innen heraus zu leuchten scheint. Der Kontrast zwischen den warmen Hautttönen und den kühlen weissen Laken, dem tiefen grünen Vorhang und dem goldenen Licht -- das ist Malerei als reines Sinneserlebnis.\n\nDas Gemälde wurde von Guidobaldo II. della Rovere, Herzog von Urbino, in Auftrag gegeben, möglicherweise als Hochzeitsgeschenk. Ob die dargestellte Frau Venus, eine Kurtisane oder eine Braut ist, wird seit Jahrhunderten diskutiert. Was auch immer sie ist -- sie beherrscht den Raum, in dem sie sich befindet -- sowohl im Gemälde als auch in den Uffizien.\n\nEbenfalls in den Uffizien: Tizians Flora, ein Porträt einer Frau mit Blumen, das zum Archetyp venezianischer Schönheit wurde, sowie mehrere Medici-Porträts.",
    biography:
      "Tiziano Vecellio wurde um 1488 in den Dolomiten Norditaliens geboren. Er zog als Kind nach Venedig und lernte bei den Gebrüdern Bellini und Giorgione. Mitte Zwanzig war er der dominierende Maler in Venedig -- eine Position, die er über 60 Jahre lang hielt.\n\nTizian arbeitete für die mächtigsten Persönlichkeiten Europas: Päpste, Kaiser, Könige und Dogen. Seine Porträts von Karl V. begründeten das Modell der Staatsporträtmalerei, das bis ins 18. Jahrhundert Bestand hatte. Er malte bis in seine spaten Achtziger weiter, wobei sein Stil immer lockerer und expressiver wurde -- einige Spätwerke wirken fast impressionistisch.",
    legacy:
      "Tizians Einfluss auf die Malerei ist immens. Rubens, Velazqüz, Rembrandt und unzählige andere studierten sein Werk. Die Venus von Urbino inspirierte direkt Manets Olympia (1863), eines der bestimmenden Gemälde der modernen Kunst. Wenn Sie das Original in den Uffizien sehen, verstehen Sie, warum sich jeder nachfolgende Maler genötigt sah, darauf zu antworten.",
    faq: [
      {
        q: "Wo ist Tizians Venus von Urbino in den Uffizien?",
        a: "Die Venus von Urbino befindet sich in Raum 83 der Uffizien. Sie ist eines der meistbesuchten Gemälde des Museums.",
      },
      {
        q: "Warum ist die Venus von Urbino berühmt?",
        a: "Sie gilt als der grösste liegende Akt der westlichen Kunst -- ein revolutionäres Werk durch seinen direkten Blick, den sinnlichen Einsatz von Farbe und seine psychologische Präsenz. Sie beeinflusste Künstler über Jahrhunderte hinweg, von Velazqüz bis Manet.",
      },
      {
        q: "Wer ist die Frau in der Venus von Urbino?",
        a: "Ihre Identität ist umstritten -- sie könnte Venus (die mythologische Göttin), eine Braut oder das Porträt einer bestimmten Frau sein. Das Gemälde wurde vom Herzog von Urbino in Auftrag gegeben, möglicherweise als Hochzeitsgeschenk.",
      },
    ],
  },
  "rembrandt": {
    name: "Rembrandt",
    title: "Rembrandt in den Uffizien | Selbstporträts - Raum 49",
    description:
      "Rembrandts Selbstporträts in den Uffizien. Die Werke des holländischen Meisters in Florenz -- Raum-49-Führer und Besucherinformationen.",
    h1: "Rembrandt in den Uffizien",
    born: "15. Juli 1606, Leiden, Niederlande",
    died: "4. Oktober 1669, Amsterdam",
    movement: "Holländisches Goldenes Zeitalter / Barock",
    ufficiRoom: "Raum 49",
    masterpieces: [
      "Selbstporträt als junger Mann (ca. 1634)",
      "Selbstporträt als alter Mann (ca. 1664)",
    ],
    intro:
      "Rembrandt mag in einer von italienischen Renaissance-Meistern dominierten Galerie unerwartet erscheinen, aber die Medici sammelten weitläufig, und die beiden Rembrandt-Selbstporträts in den Uffizien gehören zu den psychologisch durchdringendsten Werken der gesamten Sammlung. Im Abstand von dreissig Jahren gemalt, dokumentieren sie die Reise eines Mannes von jugendlichem Selbstvertraün zur gereiften Weisheit des Alters.",
    atTheUffizi:
      "Raum 49 beherbergt die nordeuropäische Sammlung der Uffizien, und Rembrandts zwei Selbstporträts sind deren Kronjuwelen. Das frühe Selbstporträt (ca. 1634) zeigt einen jungen Mann auf dem Höhepunkt seiner Schaffenskraft -- wohlhabend, selbstbewusst, in feiner Kleidung. Das späte Selbstporträt (ca. 1664) zeigt einen älteren Mann, der Bankrott, den Tod seiner Frau und seines Sohnes und beruflichen Niedergang erlebt hat. Doch der Blick ist unerschütterlich, und die Maltechnik ist meisterhaft -- breite, ausdrucksstarke Pinselstriche, die Form in Licht aufzulösen scheinen.\n\nDiese Gemälde gehören zur berühmten Sammlung von Künstler-Selbstporträts der Uffizien, die begann, als Kardinal Leopoldo de' Medici im 17. Jahrhundert mit dem Sammeln anfing. Die Sammlung umfasst heute über 1.800 Werke.",
    biography:
      "Rembrandt Harmenszoon van Rijn wurde 1606 in Leiden geboren und verbrachte den grössten Teil seines Lebens in Amsterdam. Er war der grösste Maler des Holländischen Goldenen Zeitalters -- ein Meister von Licht und Schatten (Chiaroscuro), der beispiellose psychologische Tiefe in Porträtmalerei, Historienmalerei und Selbstporträtkunst brachte.\n\nEr malte, zeichnete oder radierte im Laufe seines Lebens etwa 80 Selbstporträts -- die umfangreichste visülle Autobiografie der Kunstgeschichte. Jedes einzelne ist ein ehrliches Zeugnis des Alterns, der Emotionen und der Selbstbetrachtung. Die beiden in den Uffizien repräsentieren dieses aussergewöhnliche Projekt an zwei entscheidenden Momenten.",
    legacy:
      "Rembrandts Selbstporträts beeinflussten jeden Künstler, der versuchte, sich selbst ehrlich zu malen -- von Van Gogh über Frida Kahlo bis zu Lucian Freud. Beide Uffizien-Selbstporträts nebeneinander zu sehen, ist ein einzigartig bewegendes Erlebnis.",
    faq: [
      {
        q: "Welche Rembrandt-Gemälde gibt es in den Uffizien?",
        a: "Die Uffizien besitzen zwei Rembrandt-Selbstporträts -- eines aus seiner Jugend (ca. 1634) und eines aus dem Alter (ca. 1664). Sie befinden sich in Raum 49 zusammen mit anderen nordeuropäischen Werken.",
      },
      {
        q: "Warum gibt es Rembrandt-Gemälde in Florenz?",
        a: "Die Familie Medici sammelte Kunst aus ganz Europa. Kardinal Leopoldo de' Medici begann im 17. Jahrhundert die berühmte Sammlung von Künstler-Selbstporträts der Uffizien, die heute über 1.800 Werke umfasst.",
      },
      {
        q: "Wie viele Selbstporträts hat Rembrandt gemalt?",
        a: "Rembrandt schuf im Laufe seines Lebens etwa 80 Selbstporträts (Gemälde, Radierungen und Zeichnungen) -- die umfangreichste visülle Autobiografie der Kunstgeschichte.",
      },
    ],
  },
  "filippo-brunelleschi": {
    name: "Filippo Brunelleschi",
    title: "Brunelleschi | Architekt der Domkuppel von Florenz",
    description:
      "Filippo Brunelleschi -- Architekt der ikonischen Domkuppel von Florenz. Seine Verbindung zu den Uffizien und die Renaissance-Revolution der Architektur.",
    h1: "Filippo Brunelleschi -- Architekt des Doms",
    born: "1377, Florenz",
    died: "15. April 1446, Florenz",
    movement: "Frührenaissance",
    ufficiRoom: "Im Kontext von Raum 7 referenziert",
    masterpieces: [
      "Kuppel des Florentiner Doms (1420-1436)",
      "Ospedale degli Innocenti (1419)",
      "Basilika San Lorenzo (1421-1459)",
    ],
    intro:
      "Sie können Brunelleschis grösstes Meisterwerk von der Panoramaterrasse der Uffizien sehen -- die gewaltige Kuppel des Florentiner Doms, die sich über den Dächern der Stadt erhebt wie ein geometrisches Wunder. Brunelleschi war kein Maler, daher hängen seine Werke nicht an den Wänden der Uffizien. Aber ohne ihn hätte die Renaissance, wie wir sie kennen, vielleicht nie stattgefunden. Er erfand die Linearperspektive, das mathematische System, das jeder Maler in den Uffizien benutzte, um die Illusion von Tiefe zu erzeugen.",
    atTheUffizi:
      "Brunelleschis Verbindung zu den Uffizien ist indirekt, aber tiefgreifend. Das mathematische Perspektivsystem, das er um 1415 in Florenz demonstrierte -- mit Spiegeln und bemalten Tafeln vor dem Baptisterium -- wurde zum Fundament der Renaissance-Malerei. Jedes Gemälde in den Uffizien, das realistischen Raum zeigt, von Masaccio bis Raffael, stützt sich auf Brunelleschis Entdeckung.\n\nVon der Terrasse im zweiten Stock der Uffizien und den Fenstern entlang des östlichen Korridors können Sie seine Domkuppel die Skyline von Florenz dominieren sehen. Zwischen 1420 und 1436 ohne temporäres Holzgerüst erbaut (eine Ingenieurleistung, die jahrhundertelang nicht wiederholt wurde), bleibt die Kuppel die grösste Maürwerkskuppel, die je gebaut wurde.",
    biography:
      "Brunelleschi wurde 1377 in Florenz geboren und wurde als Goldschmied und Bildhauer ausgebildet. Im Jahr 1401 trat er gegen Lorenzo Ghiberti im Wettbewerb um den Auftrag für die Baptisteriumstüren an -- und verlor. Diese Niederlage lenkte seine Karriere in Richtung Architektur und Ingenieurwesen um, wo er die Welt verändern sollte.\n\nSeine Kuppel für den Florentiner Dom löste ein Problem, das Baumeister über ein Jahrhundert lang ratlos gemacht hatte: wie man eine 42-Meter-Öffnung ohne traditionelles Gerüst überspannt. Brunelleschi erfand eine Doppelschalenkonstruktion unter Verwendung eines Fischgräten-Ziegelmusters und genialer Hebemaschinen eigener Konstruktion. Das Projekt dauerte 16 Jahre und bleibt eine der höchsten Errungenschaften der Ingenieurskunst.",
    legacy:
      "Brunelleschi begründete die Renaissance-Architektur und erfand die Linearperspektive -- die mathematische Grundlage für die Darstellung dreidimensionalen Raums auf einer flachen Fläche. Jedes Gemälde, das Sie in den Uffizien sehen, verdankt seiner Entdeckung etwas.",
    faq: [
      {
        q: "Gibt es Brunelleschi-Werke in den Uffizien?",
        a: "Brunelleschi war Architekt, nicht Maler, daher hängen seine Werke nicht in den Uffizien. Seine Kuppel ist jedoch von der Uffizien-Terrasse aus sichtbar, und seine Erfindung der Linearperspektive ist das Fundament der Renaissance-Malerei in der gesamten Galerie.",
      },
      {
        q: "Was hat Brunelleschi gebaut?",
        a: "Sein berühmtestes Werk ist die Kuppel des Florentiner Doms (1420-1436) -- die grösste Maürwerkskuppel, die je gebaut wurde. Er entwarf auch das Ospedale degli Innocenti und die Basilika San Lorenzo in Florenz.",
      },
      {
        q: "Warum ist Brunelleschi für die Malerei wichtig?",
        a: "Er erfand um 1415 die Linearperspektive -- das mathematische System zur Erzeugung realistischer Tiefe auf einer flachen Fläche. Jeder Renaissance-Maler in den Uffizien nutzte seine Entdeckung.",
      },
    ],
  },
  "dante-alighieri": {
    name: "Dante Alighieri",
    title: "Dante Alighieri und die Uffizien | Florenz' Dichter",
    description:
      "Dante Alighieri -- der grösste Dichter von Florenz und Autor der Göttlichen Komödie. Seine Porträts in den Uffizien und seine tiefe Verbindung zur Stadt.",
    h1: "Dante Alighieri -- Florenz' Dichter in den Uffizien",
    born: "ca. 1265, Florenz",
    died: "14. September 1321, Ravenna",
    movement: "Mittelalter / Proto-Renaissance Literatur",
    ufficiRoom: "Verschiedene Räume (Porträts und Referenzen)",
    masterpieces: [
      "Göttliche Komödie (ca. 1308-1321)",
      "La Vita Nuova (ca. 1294)",
      "Porträts verschiedener Uffizien-Künstler",
    ],
    intro:
      "Dante Alighieri war kein Maler -- er war der grösste Dichter von Florenz und Autor der Göttlichen Komödie, eines der höchsten Errungenschaften der Weltliteratur. Doch seine Präsenz durchdringt die Uffizien. Sein Porträt erscheint in mehreren Gemälden in der Galerie, und sein Einfluss auf die florentinische Kultur prägte genau die Welt, die die Künstler hervorbrachte, deren Werke diese Räume füllen.",
    atTheUffizi:
      "Dante erscheint in zahlreichen Werken in den Uffizien, was seinen Status als meistverehrter Bürger von Florenz widerspiegelt. Andrea del Castagnos Freskenzyklus (ursprünglich aus der Villa Carducci) enthält ein eindrucksvolles Porträt Dantes mit seiner charakteristischen roten Kappe und dem Lorbeerkranz. Botticelli illustrierte die Göttliche Komödie mit einer Reihe von Zeichnungen, von denen einige mit den literarischen Themen in seinen Gemälden zusammenhängen.\n\nDantes Einfluss auf die Künstler der Uffizien geht über Porträts hinaus. Seine Vision des Jenseits -- das erschreckende Inferno, das hoffnungsvolle Purgatorio und das transzendente Paradiso -- inspirierte unzählige Gemälde und Skulpturen während der gesamten Renaissance. Michelangelos Jüngstes Gericht in der Sixtinischen Kapelle stützt sich stark auf Dantes Inferno, und Botticelli verbrachte Jahre damit, Illustrationen für die Göttliche Komödie zu schaffen.",
    biography:
      "Dante wurde um 1265 in Florenz geboren und gehörte einer Familie des niederen Adels an. Sein frühes Leben wurde durch seine Liebe zu Beatrice Portinari geprägt, die er zum ersten Mal sah, als beide noch Kinder waren. Obwohl sie selten sprachen und sie einen anderen Mann heiratete, wurde Beatrice Dantes lebenslange Muse. Nach ihrem Tod im Jahr 1290 kanalisierte er seinen Kummer in La Vita Nuova und schliesslich in die Göttliche Komödie.\n\nDante war auch tief in die Florentiner Politik verwickelt. Als Weisser Gülfe widersetzte er sich der päpstlichen Einmischung in die Stadtangelegenheiten. Als die Schwarzen Gülfen 1302 die Macht übernahmen, wurde Dante aus Florenz verbannt und kehrte nie zurück. Er verbrachte den Rest seines Lebens wandernd zwischen italienischen Städten und schrieb dabei das Werk, das ihn unsterblich machen sollte. Er starb 1321 in Ravenna, wo sich sein Grab bis heute befindet -- Florenz hat nie aufgehört, seine Gebeine zurückzufordern.",
    legacy:
      "Dante wird der \"Vater der italienischen Sprache\" genannt -- er schrieb die Göttliche Komödie in der Volkssprache Italienisch statt auf Latein und begründete damit den toskanischen Dialekt als Grundlage des modernen Italienisch. Sein Einfluss auf Kunst, Literatur und die Florentiner Identität ist unermesslich.",
    faq: [
      {
        q: "Gibt es Dante-Gemälde in den Uffizien?",
        a: "Dante erscheint als Motiv in mehreren Gemälden in den Uffizien, darunter Porträts von Andrea del Castagno und anderen Florentiner Künstlern. Als Dichter hat er selbst keine bildende Kunst geschaffen.",
      },
      {
        q: "Warum ist Dante für Florenz so bedeutend?",
        a: "Dante ist der berühmteste Bürger von Florenz und der \"Vater der italienischen Sprache\". Seine Göttliche Komödie (1308-1321) wurde auf Toskanisch-Italienisch statt auf Latein verfasst und legte das Fundament für das moderne Italienisch. Sein Einfluss prägte die kulturelle Welt, die die Künstler der Uffizien hervorbrachte.",
      },
      {
        q: "Wurde Dante aus Florenz verbannt?",
        a: "Ja. Im Jahr 1302 wurde Dante verbannt, nachdem seine politische Fraktion die Macht verloren hatte. Er kehrte nie nach Florenz zurück und starb 1321 in Ravenna. Florenz hat wiederholt die Rückgabe seiner sterblichen Überreste gefordert.",
      },
    ],
  },
  "piero-della-francesca": {
    name: "Piero della Francesca",
    title: "Piero della Francesca | Herzog von Urbino - Raum 8",
    description:
      "Piero della Francescas berühmte Porträts des Herzogs und der Herzogin von Urbino in den Uffizien, Raum 8. Renaissance-Porträtkunst auf höchstem Niveau.",
    h1: "Piero della Francesca in den Uffizien",
    born: "ca. 1415, Borgo Sansepolcro, Toskana",
    died: "12. Oktober 1492, Borgo Sansepolcro",
    movement: "Frührenaissance",
    ufficiRoom: "Raum 8",
    masterpieces: [
      "Porträts des Herzogs und der Herzogin von Urbino (ca. 1465-1472)",
    ],
    intro:
      "Piero della Francescas Doppelporträt von Federico da Montefeltro und seiner Frau Battista Sforza ist eines der ikonischsten Bilder der italienischen Renaissance. Im Profil vor einer weiten umbrischen Landschaft gemalt, blicken der Herzog und die Herzogin einander über den Rahmen hinweg mit der gelassenen Würde römischer Münzen an. Das Gemälde handelt ebenso von Mathematik und Licht wie von Macht.",
    atTheUffizi:
      "Raum 8 beherbergt eines der bekanntesten Werke der Uffizien: das Diptychon (zweiteiliges Gemälde) des Herzogs und der Herzogin von Urbino. Die Tafeln sind beidseitig bemalt -- die Vorderseite zeigt die berühmten Profilporträts, während die Rückseite beide Figuren auf Triumphwagen mit allegorischen Figuren zeigt.\n\nFederico da Montefeltro, Herzog von Urbino, war einer der erfolgreichsten Feldherren des 15. Jahrhunderts. Er wird stets im linken Profil gezeigt -- immer -- weil er bei einem Turnier sein rechtes Auge und den Nasenrücken verloren hatte. Anstatt die Entstellung zu verbergen, arbeitete Piero sie mit charakteristischer Ehrlichkeit in das Porträt ein.\n\nDie Landschaften hinter beiden Figuren sind aussergewöhnlich. Piero gibt die sanften Hügel, Flüsse und Städte der Region Marken mit einer Präzision und Leuchtkraft wieder, die die holländische Landschaftsmalerei um zwei Jahrhunderte vorwegnimmt. Das Licht ist perfekt kalibriert -- alles badet in einer klaren, kühlen Atmosphäre, die ein Gefühl unendlichen Raums erzeugt.\n\nPiero war auch Mathematiker, der Abhandlungen über Perspektive und Geometrie schrieb. Seine Gemälde spiegeln diese doppelte Leidenschaft wider -- jedes Element ist präzise berechnet, doch das Ergebnis wirkt natürlich und gelassen statt steif.",
    biography:
      "Piero wurde um 1415 in der toskanischen Stadt Borgo Sansepolcro geboren, studierte in Florenz, verbrachte aber einen Grossteil seiner Karriere an Höhen in ganz Mittelitalien. Sein grösster Freskenzyklus -- die Legende des Wahren Kreuzes in Arezzo -- ist eines der Meisterwerke der Frührenaissance.\n\nPiero war ein wahrer Renaissance-Universalgelehrter: Maler, Mathematiker und Theoretiker. Seine Abhandlungen über Perspektive (De Prospectiva Pingendi) und Geometrie beeinflussten Künstler und Mathematiker über Generationen. In seinen späteren Jahren erblindete er und hörte auf zu malen, um sich bis zu seinem Tod im Jahr 1492 ganz der Mathematik zu widmen.",
    legacy:
      "Piero della Francesca war nach seinem Tod weitgehend vergessen und wurde erst im 20. Jahrhundert wiederentdeckt. Heute gilt er als einer der grössten Maler der Frührenaissance, bewundert für seine leuchtende Farbgebung, geometrische Präzision und tiefe Ruhe.",
    faq: [
      {
        q: "Wo ist Piero della Francescas Gemälde in den Uffizien?",
        a: "Das Doppelporträt des Herzogs und der Herzogin von Urbino befindet sich in Raum 8. Die Tafeln sind beidseitig bemalt -- verpassen Sie nicht die Rückseite mit den Triumphzügen.",
      },
      {
        q: "Warum wird der Herzog im Profil gezeigt?",
        a: "Federico da Montefeltro verlor bei einem Turnier sein rechtes Auge und den Nasenrücken. Er wird stets von der linken Seite gezeigt. Piero arbeitete dies mit charakteristischer Direktheit in das Porträt ein.",
      },
      {
        q: "Was macht Piero della Francesca besonders?",
        a: "Piero verband Malerei mit Mathematik -- seine Werke zeichnen sich durch eine einzigartige Leuchtkraft, geometrische Präzision und ein Gefühl der Ruhe aus. Er schrieb auch einflussreiche Abhandlungen über Perspektive, die die Renaissance-Kunst prägten.",
      },
    ],
  },
  caravaggio: {
    name: "Caravaggio",
    title: "Caravaggio in den Uffizien | Medusa, Bacchus & Opfer Isaaks",
    description:
      "Sehen Sie Caravaggios Medusa, Bacchus & Opfer Isaaks in den Uffizien Saal 90. Dramatische barocke Meisterwerke von Licht und Schatten.",
    h1: "Caravaggio in der Uffizien-Galerie",
    born: "29. September 1571, Mailand",
    died: "18. Juli 1610, Porto Ercole, Toskana",
    movement: "Barock",
    ufficiRoom: "Saal 90 (Erstes Stockwerk)",
    masterpieces: [
      "Medusa (um 1597)",
      "Bacchus (um 1598)",
      "Opfer Isaaks (um 1603)",
    ],
    intro:
      "Betreten Sie Saal 90 und Caravaggio schlägt Ihnen ins Gesicht. Nach der ätherischen Schönheit Botticellis und der gelassenen Perfektion Raffaels ist Caravaggios Medusa — ihr Mund in einem Schrei erstarrt, Blut strömt aus ihrem abgetrennten Hals — ein Schock. Dies ist Kunst, die nicht um Ihre Bewunderung bittet. Sie fordert Ihre Aufmerksamkeit.",
    atTheUffizi:
      "Die Caravaggio-Sammlung der Uffizien befindet sich in Saal 90 im ersten Stock, ein dramatischer Kontrast zu den Renaissance-Meisterwerken im Obergeschoss. Diese drei Gemälde repräsentieren Caravaggio auf dem Höhepunkt seiner Kräfte, bevor sein gewalttätiges Temperament ihn zur Flucht aus Rom zwang.\n\nMedusa (um 1597) ist auf einem runden Holzschild gemalt — nicht auf Leinwand. Sie wurde von Kardinal Francesco Maria del Monte als Geschenk für den Grossherzog der Toskana in Auftrag gegeben. Die Gorgone wird im Moment ihrer Enthauptung durch Perseus gezeigt, Mund offen in einem stummen Schrei, Schlangen winden sich in ihrem Haar. Blut spritzt aus ihrem Hals. Was es noch verstörender macht: Es ist ein Selbstporträt. Caravaggio malte sein eigenes Gesicht im Moment des Todes.\n\nBacchus (um 1598) zeigt den Gott des Weins als Strassengauner — mit nackten Schultern, leicht betrunken, der Ihnen ein Glas Wein mit einem wissenden Grinsen anbietet. Das Obst auf dem Tisch fault. Dies ist keine mythologische Grösse — es ist Realität auf ein klassisches Thema geschlagen. Schauen Sie genau auf die Weinkaraffe: Sie können Caravaggios winziges Selbstporträt im Glas reflektiert sehen.\n\nOpfer Isaaks (um 1603) fängt den schrecklichen Augenblick ein, bevor Abrahams Messer in den Hals seines Sohnes stösst. Das Gesicht des Jungen ist vor Entsetzen erstarrt, sein Mund offen in einem Schrei, der die Medusa widerspiegelt. Die Hand des Engels stoppt die Klinge im letzten möglichen Moment. Das Lamm — das Ersatzopfer — wartet geduldig in der Ecke. Es ist unerträglich spannungsgeladen.\n\nCaravaggios revolutionäre Technik — dramatisches Chiaroscuro (Licht, das aus der Dunkelheit hervortritt) — ist hier voll zur Schau gestellt. Körper tauchen aus schwarzen Hintergründen auf wie Schauspieler im Scheinwerferlicht auf einer Bühne. Jedes Detail wird mit brutaler Ehrlichkeit wiedergegeben: schmutzige Fingernägel, faltige Haut, das Gewicht des Fleisches.",
    biography:
      "Michelangelo Merisi da Caravaggio wurde 1571 während eines Pestausbruchs in Mailand geboren. Sein Vater starb, als er jung war. Er lernte in Mailand, zog dann um 1592 nach Rom, wo er in Armut lebte und Genreszenen und Stillleben malte.\n\nSein Durchbruch kam, als Kardinal Francesco Maria del Monte um 1595 sein Mäzen wurde. Der Kardinal gab religiöse Werke und mythologische Themen in Auftrag, die Caravaggios revolutionären Stil zeigten. Er malte direkt von lebenden Modellen — oft Prostituierte, Strassenmenschen und Kriminelle — ohne vorherige Zeichnungen. Was Sie sehen, ist das, was vor ihm war.\n\nAber Caravaggio war so gewalttätig wie seine Gemälde. Er prügelte sich ständig, wurde mehrfach verhaftet und tötete 1606 einen Mann bei einer Auseinandersetzung über ein Tennisspiel. Er floh aus Rom und verbrachte seine letzten vier Jahre auf der Flucht — Malta, Sizilien, Neapel — und malte zunehmend dunkle und verzweifelte Werke. Er starb 1610 im Alter von 38 Jahren, möglicherweise an Malaria, möglicherweise an einer Wundinfektion, möglicherweise ermordet. Sein Körper wurde nie gefunden.\n\nTrotz seines kurzen, chaotischen Lebens revolutionierte Caravaggio die europäische Malerei. Sein dramatischer Einsatz von Licht und Schatten, sein kompromissloser Realismus und seine psychologische Intensität beeinflussten Generationen von Künstlern. Die 'Caravaggisti' — seine Anhänger — verbreiteten seinen Stil in ganz Europa.",
    legacy:
      "Caravaggio starb in Dunkelheit und war 200 Jahre lang weitgehend vergessen. Seine Arbeit wurde als zu schockierend, zu realistisch, zu dunkel angesehen. Das 20. Jahrhundert entdeckte ihn wieder, und heute gilt er als einer der einflussreichsten Maler der westlichen Kunst. Seine Chiaroscuro-Technik beeinflusste Rembrandt, Velázquez und nahezu jeden Barockmaler. Seine psychologische Intensität ebnete den Weg für den modernen Realismus.",
    faq: [
      {
        q: "Wo sind Caravaggios Gemälde in den Uffizien?",
        a: "Caravaggios Werke befinden sich in Saal 90 im ersten Stock (Erdgeschoss in europäischer Zählung). Dieser Saal zeigt barocke Kunst und ist von den Hauptgalerien der Renaissance im Obergeschoss getrennt.",
      },
      {
        q: "Warum ist die Medusa auf einem Schild?",
        a: "In der griechischen Mythologie benutzte Perseus seinen polierten Schild als Spiegel, um nicht direkt auf Medusa zu schauen (deren Blick Menschen zu Stein verwandelte). Nach ihrer Enthauptung befestigte er ihren Kopf an seinem Schild. Caravaggio malte auf einem echten zeremoniellen Schild für Authentizität.",
      },
      {
        q: "Ist die Medusa ein Selbstporträt?",
        a: "Ja. Caravaggio verwendete sein eigenes Gesicht für Medusas Züge. Das war für ihn nicht ungewöhnlich — er fügte sich oft in seine Gemälde ein, manchmal auf schockierende Weise.",
      },
      {
        q: "Warum ist Caravaggio so berühmt?",
        a: "Caravaggio revolutionierte die Malerei mit seinem dramatischen Einsatz von Licht und Schatten (Chiaroscuro), kompromisslosem Realismus und psychologischer Intensität. Er malte direkt aus dem Leben ohne vorherige Skizzen und schuf Werke von schockierender Unmittelbarkeit. Sein Einfluss auf die Barockkunst war immens.",
      },
      {
        q: "War Caravaggio ein Krimineller?",
        a: "Ja. Caravaggio wurde häufig wegen Schlägereien, Vandalismus und Körperverletzung verhaftet. 1606 tötete er einen Mann bei einer Auseinandersetzung und verbrachte seine letzten vier Jahre auf der Flucht vor der Justiz. Er starb auf der Flucht im Jahr 1610.",
      },
    ],
  },
};
